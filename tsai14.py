"""The high level control objects that warps visa layers.
...
class KeySightEXA:
    ...
    # both (add `?` to query):
    # :BANDWIDTH:RESOLUTION:AUTO (0|1)|?  ->  auto decide bandwidth resolution
    # :BANDWIDTH:RESOLUTION  (value)  ->  bandwidth resolution
    # :SENS:FREQ:CENT (freq)|?  ->  center frequency of center-span
    # :SENS:FREQ:SPAN (freq)|?  ->  span frequency of center-span
    # :SENS:FREQ:STAR (freq)|?  ->  start frequency of start-stop
    # :SENS:FREQ:STOP (freq)|?  ->  stop frequency of start-stop
    # :SENS:SWE:POIN (point)|?  ->  number of sweep points
    # :SENS:AVER:COUN (count)|?  ->  number of avaerges